...
class CassandraConnection
{
	...
	function filtered_read($keyspace, $filters, $column_family, $consistency)
	{
		...
		do
		{
			...
			try
			{
				...
				$start = microtime(true);
				$indexes = $this->get_client()->get_indexed_slices($columnParent,
							 $indexClause,
							 $predicate,
							 $consistency);
				$end = microtime(true);
	...
	function read_with_indexes($keyspace, $column_family, $indexes, $columns=null, $range = null, $consistency)
	{
		...
		$iterator = $columnfamily->get_indexed_slices($clause,
					 $columns,
					 "",
					 "",
					 false,
	...
	function read($keyspace, $key, $column_family, $consistency)
	{
		...
		try
		{
			$indexes = $this->client->multiget_slice(
						 array($key),
						 new cassandra_ColumnParent(array("column_family" => $column_family)),
						 new cassandra_SlicePredicate(array("slice_range" => new cassandra_SliceRange(array("start" => "", "finish" => "", "count" => 100)))),
						 $consistency);
		} catch (Exception $e)
		{
