...
class RSS
{
	...
	static function insert_rss_article_comment($id)
	{
		$rss_link = "comments/" . $id . ".xml";

		$rss_link = "http:///rss/" . $rss_link;
	...
	static function getRSSFeedForArticleCategory(ArticleCategory $articleCategory)
	{
		$rss_link = ($articleCategory->getSubcategoryName() == '') ? $articleCategory->getCategoryName() . '.xml' : $articleCategory->getCategoryName() . ':' . $articleCategory->getSubcategoryName() . '.xml';
...
<?
	...
	public static function insertRSSLinksForArticleCategory(ArticleCategory $articleCategory)
	{
		$rss_link = self::getRSSFeedForArticleCategory($articleCategory);
...
<?
	...
	public static function getRSSLinksForArticleCategory(ArticleCategory $articleCategory)
	{
		$rss_link = self::getRSSFeedForArticleCategory($articleCategory);
		$links = array();
		$links[] = '<a target="_new" href="' . $rss_link . '" ><img src="http://img./spriting/trans.gif" class="sprite s_xml" alt=""></a>';
		$sites = self::getSites($rss_link);
		foreach ($sites as $site => $info)
		{
			$links[] = '<a target="_new" href="' . $info["link"] . '"><img src="http://img./spriting/trans.gif" class="sprite ' . $info["image_sprite"] . '" alt="' . $info["alt"] . '"></a>';
		}
		return $links;
	}
