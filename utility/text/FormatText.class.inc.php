...
class FormatText {
    ...
    static function formatText($text, $addPara = 1, $addBreakLine = 0, $removeHardBreak = 1, $stripTags = 0, $addHardBreak = 0, $allowedTags = '') {
        ...
        if ($addPara) {
            ...
            foreach ($textElements as $item) {
                ...
                $item = trim($item);
                if (strlen($item) != 0) {
                    if (preg_match("/<(ol|ul|li)>/", $item)) {### don't wrap list items
                        $newText .= "$item";
                    }
                    else {
                        $newText .= "<p>$item</p>";
        ...
        if ($removeHardBreak) { ### remove hardreturns
            $text = str_replace("\n", " ", $text);
	...
	static function rssfilter($str,$isTitle=false) {
		...
		if($isTitle) { $str = self::rssfilter_clean_title($str); }
	...
	","\n",$str);
		...
		if($isTitle) { $str = self::rssfilter_format_title($str); }
	...
	public static function dedupelinks($str)
	{
		...
		foreach ($matches[1] as $value) {
			...
			if($label_tmp==$value_tmp){$str=str_replace("[$value]",'',$str);}
				$pos++;
		}
