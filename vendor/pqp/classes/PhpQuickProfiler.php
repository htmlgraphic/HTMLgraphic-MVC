...
class PhpQuickProfiler extends Controller{
	
	public $output = array();
	public $config = '';
	
	public function __construct($startTime) {
		$this->startTime = self::getMicroTime($startTime);
		$this->config = Config::get('DocRoot').'/vendor/pqp';
	...
	public function fileCompare($info,$info2)
	{
		return $info["age"] == $info2["age"] ? 0 : $info["age"] > $info2["age"] ? -1 : 1;
	}
	...
	public function gatherQueryData() {
		...
		$queryTotals['count'] += count(Debugger::$queries);
		foreach(Debugger::$queries as $key => $query) {
			$query = $this->attemptToExplainQuery($query);
			$queryTotals['time'] += $query['time'];
			$query['raw_time'] = $query['time'];
	...
	public function display($db = '') {
		...
		$this->loadView("/view/profiler/Profiler",array("output" => $this->output,"config" => $this->config));
